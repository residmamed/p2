...
**Lens Sourcing**:
The second, browserless photo → suppliers pipeline, behind `POST /api/find-suppliers` (`app/lens_suppliers.py`). Two REST calls and nothing else: SerpApi's Google Lens finds product pages hosting a matching image, then Oxylabs' Web Scraper API opens each Alibaba, 1688 and Made-in-China hit for supplier name, price and MOQ. Answers in seconds where [[image-sourcing]] takes minutes, because no browser drives an upload widget and no actor run is queued. In exchange it only finds what Lens has indexed — coverage of Chinese B2B listings is partial and varies by category, so an empty `results` here does not mean no supplier exists, and the response says so. Step 1 is cached 30 days by image hash; step 2 never is, because a month-old candidate URL is fine and a month-old price is not. Not a replacement for [[image-sourcing]] — a faster first look, which is why both endpoints exist. Which sites it can actually reach is measured, not assumed: see [[lens-site-coverage]].
...
**Lens Site Coverage**:
Which supplier sites [[lens-sourcing]] can actually reach, counted rather than assumed. Measured 2026-07-30 over the 36 searches in the 30-day Lens cache (12,040 candidates, tallied by hostname): **Alibaba 89, Made-in-China 19 (all 19 reachable), Taobao 1, 1688 zero**. Made-in-China is therefore enriched like Alibaba — its JSON-LD `brand` carries the factory's own name, so it needs no bespoke reader — except that a hit must match `/product/` or `/product_` to count, since Lens mixes category, keyword-search and video URLs in with listings and enriching one yields a supplier row with no company behind it. **1688's zero is structural**: it is Alibaba's domestic site and sits outside Google's index, so no photo will ever produce a 1688 row through Lens. Its listings are reachable only through [[image-sourcing]]'s Apify actor, so coverage is judged per *selected* site (`LENS_BLIND_SITES`) — a product missing a site Lens is blind to gets a deep search for that site alone, merged into the group the fast path already built. Related: the enrichment budget is dealt round-robin by site, because the site with the most hits is not the site with the best ones.
_Avoid_: Lens coverage (ambiguous with [[photo-search-google-lens]]), site support (implies a config flag, not a measurement)
