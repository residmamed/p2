...
### Which sites Lens can actually reach

Counted 2026-07-30 over the 36 searches then in the 30-day Lens cache — 12,040 candidates, tallied by hostname:

| site | Lens hits | reachable | in this pipeline |
|---|---|---|---|
| Alibaba | 89 | yes | enriched |
| Made-in-China | 19 | 19/19 | enriched |
| Taobao | 1 | yes | returned, not enriched |
| **1688** | **0** | — | **unreachable via Lens** |

Two things follow, and both are now wired rather than assumed.

**Made-in-China is a first-class target.** It is the second-best-served site here, and unusually every one of its hits carried a real destination rather than one of the `lens.google.com/goto` wrappers that make most `exact_matches` unusable. It also parses without a bespoke reader: the generic og:/JSON-LD layers already yield title, image, price and — via JSON-LD `brand` — the factory's own name, which is the field Alibaba needs a hydration-blob reader for. Its company link comes from the page's `com-name` anchor, since the site ships no `companyProfileUrl`. The one caveat is shape: Lens mixes category, keyword-search and `/video-channel/` URLs in with listings (6 of the 19), and enriching one parses a directory page into a supplier row with a plausible title and no company behind it — so a MIC hit must match `/product/` or `/product_` to become a result, and the rest are reported as context.

**1688 cannot be reached this way at all.** Zero is not a slow category or a bad photo: 1688.com is Alibaba's *domestic* site and sits almost entirely outside Google's index. The listings do exist and `/api/sourcing/by-url` finds them through the Apify `devcake/scraper-by-image` actor — but that path used to run only for products the fast path missed *entirely*, so any product where Lens found an Alibaba listing never queried 1688 at all. A user with 1688 selected could search all day and never see one. Coverage is now judged **per selected site** (`LENS_BLIND_SITES` in `frontend/src/api.js`): a product that came back without a site Lens is blind to gets a deep search for *that site only*, merged into the group the fast path already built. Kept to the sites with a measured zero — a site Lens does serve, missing from one product, is an ordinary miss and not worth minutes of browser time.

**The enrichment budget is dealt round-robin by site.** `MAX_ENRICH` is 10 and Lens routinely returns far more. Taking the first ten of the Lens ordering spends the lot on whichever site returned most: measured on a live 40oz-tumbler search — 58 marketplace hits, 55 Alibaba and 3 Made-in-China — all ten slots went to Alibaba and every Made-in-China row came back with no supplier and no MOQ. Volume is not quality; Lens returns more Alibaba because Google indexes more Alibaba. Dealing a slot to each site in turn (same rule as `sourcing._vision_candidates`) enriched all three, naming FOSHAN WISDOM HOUSEWARE CO.,LTD and Anhui Beauty Home Technology Co., Ltd at $2.92 and $3.50/500 PCS.

> **Still unverified:** the 1688 and Taobao selectors in `marketplace_product.py`. Lens has never returned a reachable listing on either site, so nothing has exercised them; they are written from the page's documented shape and follow the `supplier_resolve.VERIFIED` convention. A miss there costs one field on one row — the caller falls back to SerpApi's data and says so. The Made-in-China path is no longer in that category: it is pinned in `tests/test_made_in_china_lens.py` against a live product page fetched through Oxylabs.
...
- **Suppliers are prefetched silently after a product search — both passes, every product.** When results land, the supplier lookup starts in the background for the whole grid and renders nothing: no progress bar, no rows, no warnings. Suppliers still appear only when the user asks for them; the point is purely that the asking is then fast. The caches in `api.js` are keyed per product photo, so the click either finds the answer already there or joins the request in flight rather than re-issuing (and re-billing) it. The prefetch runs on its own AbortController so a new search cancels it, and is cleared when a new search starts so a previous query can never serve its suppliers.
  ...
  - **It is paced against the Browserbase plan, not against a guess.** The pacing used to be strictly serial, on the belief that one deep search holds three cloud browsers for its three sites. That is only the worst case. On the by-URL path this endpoint actually takes, [`_run_site`](backend/app/sourcing.py) sends Alibaba and 1688 to **Apify actors, which open no browser at all** — only Made-in-China drives one. So a product normally costs *one* session, and three only when both Apify actors come back empty and fall back to the browser upload. `PREFETCH_PARALLEL` (6) products now run at once against a Developer plan's 25 concurrent browsers: six sessions normally, eighteen at the worst, deliberately short of the cap because going over does not queue — the session create fails outright. **On a Free plan (3 concurrent) this must be 1.** Workers pull from a shared cursor rather than running in chunks, because chunked batches idle at every boundary waiting for their slowest member and these searches differ by minutes.
  - **The run ends when every product has an answer, not when the list runs out.** A search that *threw* — a captcha'd browser session, a Browserbase session refused because the plan's three were busy, a timeout — is not an answer: it says nothing about whether the product has suppliers, yet it used to leave the product looking exactly like one that was searched properly and came back empty. `findSuppliersByImage` now returns the products it could not resolve (distinct from the ones it resolved as empty), and the background run gathers them up and goes round again, up to `PREFETCH_ROUNDS` times with `PREFETCH_RETRY_MS` between rounds — the failures worth retrying are contention, and contention clears given a moment. **"Searched, found nothing" is never retried**: it is a real answer, the request would be identical, and a second attempt would reliably produce only a second bill. Retries are bounded because some failures are permanent, and an unbounded retry on one of those is an infinite loop that bills for every turn. If a product defeats every round, the corner ring stops short of full rather than claiming completion.
  - **The per-card dot means the deep pass is done for that card, not the fast one.** It is set only from the deep loop. When the prefetch covered just the first six products deeply but marked twelve, the seventh through twelfth wore a finished-looking mark with minutes of marketplace search still owed on them, and pressing the button went straight into that search for exactly the items the grid had called done. Every product now goes through both passes, and no product is marked until its own deep search has returned — so a full grid of dots (and the corner ring's tick) means the button has nothing left to do.
...
- **Prices and ratings are read by 25 cloud browsers at once.** `SUPPLIER_ENRICHMENT_BACKEND=browserbase` opens each matched product page in its own Browserbase session instead of fetching it through Oxylabs. Slower per page — measured 9.7-33s against Oxylabs' sub-second fetch — and chosen for reach, not speed: a real browser sees the page state that carries Alibaba's `averageStar`, which a raw fetch does not. The latency is answered with width, `BROWSERBASE_PAGE_CONCURRENCY=25`, so a whole search costs about as long as its slowest page.
  - **One session per page, not one session with many tabs.** Tabs would be cheaper (sessions bill per second and each create costs seconds), but every tab shares its session's exit IP, and these are sites this codebase already has a bot-detection history with. A session per page is a proxy IP per page. The concurrency is the plan's hard cap — over it the session create *fails* rather than queueing (3 on Free, 25 on Developer).
  - **Ratings are Alibaba-only, and that is the site's doing.** Alibaba embeds `averageStar` / `totalReviewCount`; Made-in-China publishes no rating in any form — no JSON-LD `aggregateRating`, no rating-shaped key. `None` therefore means "this site does not say" and must render as N/A: the grid sorts on this column, and a fabricated 0 would rank an unrated factory below every genuinely bad one. Live run: 8 of 11 rows rated, all 8 Alibaba.
  - **`_fill` is the whole contract for parsed fields, and it bit.** Rating extraction worked on the first live page it was pointed at; `_fill` then dropped it, because that function copies a hardcoded list of field names. The parser returned `rating=None` on a page serving `"averageStar":4.6` — indistinguishable from a site that publishes no rating, and it very nearly shipped as "Alibaba has no ratings". Add the field to `_fill` when you add it to `ParsedProduct`.
  - **Alibaba category pages are now rejected.** The code assumed Alibaba's Lens hits were listings or nothing. A live search returned `alibaba.com/countrysearch/CN/...`, which enriched into a real supplier name, a "50 sets" MOQ and a price of **$1,000,000** — every field plausible, the whole row about a directory. Alibaba is tested with a deny-list of directory paths while Made-in-China keeps its allow-list of listing paths: MIC's listing shapes are known and its directories common, Alibaba's are the reverse, and dropping a real factory is worse than admitting a rare category page.
  - **A config default must never make the test suite spend money.** Switching the backend to Browserbase silently turned every enrichment test into a live cloud-browser call — the suite went from 1.7s to 144s, ten tests failed on network behaviour, and each run billed real sessions. `tests/conftest.py` now pins the backend and the page cap for the whole suite, so the browser path has to be asked for explicitly.
- **Supplier search calls SerpApi and nothing else.** Two switches, both on by default, reduce the supplier pipeline to a single vendor:
  - `DEEP_SEARCH_ENABLED = false` ([api.js](frontend/src/api.js)) drops the fallback to the marketplaces' own reverse-image indexes, so no Apify actor runs and no cloud browser is driven. Gated through one derived `useDeep`, so a caller's request and the app-wide switch can never disagree in only some of the three places the deep pass is reached.
  - `SUPPLIER_SEARCH_SERPAPI_ONLY=true` ([config.py](backend/app/config.py)) skips the Oxylabs enrichment step even where Oxylabs is configured and working, and skips the contact lookup whatever the caller asked for — that step reads supplier sites through Oxylabs and judges their images with a vision call, and it has nothing to work from anyway since it starts from a supplier this mode never identifies.
  - **What the rows lose.** Every candidate still comes back — a thin record is still a lead, and dropping it would turn "we didn't look" into "there is nothing here" — flagged `enriched: false` with the reason. Gone is everything only the product page carries: the factory's name, the MOQ, and the price. Measured over four live generic-product searches on 2026-07-30: 0, 7, 15 and 12 marketplace rows, and across all 34 — **zero supplier names, zero MOQs, one price**. What remains on a marketplace row is a title, a thumbnail and a link.
  - **SerpApi cannot supply the price, and it is worth knowing why.** Lens *has* a price field, so it is natural to assume the data is there and merely unread. It is not: on a live `visual_matches` call, 10 of 60 matches carried a price and **all 10 were Amazon or Walmart**, while every Alibaba match had `price: null`. Lens reads prices off retail pages, not Chinese marketplace ones — so on exactly the sources this feature exists for, there is nothing to carry. `_candidate_from` already handles both shapes of the field; the field is simply absent.
  - **Coverage is the real cost, not speed.** 1688 becomes unreachable — over the 36 searches in the Lens cache, Lens returned alibaba 89 hits, made-in-china 19, taobao 1, **1688 zero** — and branded retail products return nothing at all: a live search on a Stanley Quencher found 390 Lens matches and not one on any Chinese marketplace. Generic goods do fine; branded ones do not. Both cases now say so in a warning rather than showing an unexplained empty grid.
  - **SerpApi quota is the binding constraint.** A Lens search costs **2** calls and the prefetch runs one per product across the whole grid, so a Free plan's 250/month is ~125 products — one search returning 50 products spends 40% of the month. `GET /api/health` reports the account count; add `SERPAPI_KEY_2` to raise it.
- **Each metered vendor can hold more than one account.** Zyte, Apify, SerpApi and Oxylabs are read through [`app/credentials.py`](backend/app/credentials.py) rather than from `config.settings`, because each meters something the pipeline is capable of exhausting — a prefetch over a full grid is one Lens search per product plus an actor run per product per site. Extra accounts are configured as numbered siblings of the existing name (`SERPAPI_KEY`, `SERPAPI_KEY_2`, …) and handed out round robin; Oxylabs pairs positionally, since it identifies an account by username *and* password. Numbered rather than comma-separated because two of these values are passwords and any delimiter is a character some password contains. Setting none of them behaves exactly as a single key always did.
  - **A pool spreads load; it does not fail over.** Nothing retries a rejected request on the next account. The pool raises the ceiling — it cannot make a request that was refused succeed.
  - **Rotation stops at a multi-step flow.** An Apify run started on account A does not exist on account B: polling it there 404s and the dataset reads back empty, which the pipeline would report as "no suppliers found" rather than as a fault. `google_lens.py` therefore picks one token and threads it through start → poll → fetch, instead of calling `next()` at each step. Same reasoning, one step coarser, for `ZyteClient` and `OxylabsClient`, which take an account per client rather than per request so one search's burst stays attributable to one account.
  - **`GET /api/health` reports how many accounts each vendor loaded** — counts only, never the keys, since it is unauthenticated. Worth having because a second account that silently failed to load is invisible: the pipeline still answers, just at the old ceiling, and nothing else would say which.
  - **What this does not buy is a faster single search.** The Lens pass already fires every product at once against one key, so extra SerpApi keys raise quota and nothing else. Wall-clock time moves with `PREFETCH_PARALLEL` and the Browserbase plan behind it.
